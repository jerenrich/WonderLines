...
Debug builds default to `COLORING_MODE = mock`. Choose an iPad simulator and press Run. Enter a description, select a child’s age, and tap **Make 5 demo sheets**. The Settings button in the toolbar lets you choose Flare or Sunburst and 1–5 images per generation. Both settings are saved locally; the defaults are Sunburst and five images. Sample flowers arrive progressively, with no network call or charge. The first launch has no assumed age. Subsequent launches restore the selected age locally.
