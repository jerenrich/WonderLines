...
Debug builds default to `COLORING_MODE = mock`. Choose an iPhone or iPad simulator and press Run. Enter a description, choose an age difficulty in Settings, and tap **Make 5 demo sheets**. The Settings button in the toolbar lets you adjust age difficulty, choose from eleven image models (Flare, Sunburst, and nine Cloudflare-hosted alternatives), and select 1–5 images per generation. These settings are saved locally; the defaults are Sunburst and five images. Sample flowers arrive progressively, with no network call or charge. The first launch has no assumed age. Subsequent launches restore the selected age locally.
